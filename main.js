...
ipcMain.handle('start-scan', (event, scanType, customPath) => {
  const defenderPath = `"C:\\Program Files\\Windows Defender\\MpCmdRun.exe"`;
  let scanArgs = ['-Scan', '-ScanType'];
  switch (scanType) {
    case 'Quick': scanArgs.push('1'); break;
    case 'Full': scanArgs.push('2'); break;
    case 'Custom':
      if (customPath) {
        scanArgs.push('3', '-File', `"${customPath}"`);
      } else {
        scanArgs.push('1');
      }
      break;
  ...
  const fullCommand = `${defenderPath} ${scanArgs.join(' ')}`;
  const sudoOptions = { name: 'TForce Anti Virus' };
  sudo.exec(fullCommand, sudoOptions, (error, stdout) => {
    ...
    const match = output.match(/Threats found:\s*(\d+)/);
    const threatCount = match ? parseInt(match[1]) : 0;
    if (threatCount > 0) {
      mainWindow.webContents.send('scan-update', {
        type: 'threat',
        filePath: 'See Windows Security for details',
        threatName: `${threatCount} threat(s) detected`
      });
    }
...
ipcMain.handle('get-system-stats', () => {
  return new Promise((resolve) => {
    ...
    const timeout = setTimeout(() => { worker.terminate(); resolve({ error: true, message: "Stats worker timed out." }); }, 1500);
    worker.on('message', (response) => {
      clearTimeout(timeout);
      resolve(response.success ? response.data : { error: true, message: response.error });
      worker.terminate();
    });
    worker.on('error', (err) => {
      clearTimeout(timeout);
